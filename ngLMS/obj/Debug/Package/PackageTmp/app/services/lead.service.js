...
var LeadService = /** @class */ (function () {
    ...
    LeadService.prototype.getReminderCount = function () {
        return this.http.get('/api/Lead/GetReminderCount', this.getToken())
            .map(function (data) {
            // login successful if there's a jwt token in the response
    ...
    LeadService.prototype.getReminders = function () {
        return this.http.get('/api/Lead/GetReminderForLoginUser', this.getToken())
            .map(function (data) {
            // login successful if there's a jwt token in the response
            if (data && data.status === 200) {
                return data.text();
    ...
    LeadService.prototype.getAllReminders = function () {
        return this.http.get('/api/Lead/GetAllReminderForLoginUser', this.getToken())
            .map(function (data) {
            // login successful if there's a jwt token in the response
            if (data && data.status === 200) {
                return data.text();
