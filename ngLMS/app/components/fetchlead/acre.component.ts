...
export class AcreComponent implements OnInit {
    ...
    getLastFetchDetails() {
        this.isLoading = true;
        this.Lead.getLastFetch('2').subscribe(
            data => {
    ...
    fetchLead() {
        ...
        if (datDif > 30) {
            this.alert.error("30 Days record is allowed at a time", 5000);
            return false;
        }
        this.isLoading = true;

        this.Lead.getAcreLead(this.apiUserID, this.apiPassword, this.searchFromDt, this.searchToDt).subscribe(
            data => {
